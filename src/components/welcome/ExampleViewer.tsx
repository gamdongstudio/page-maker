...
export function ExampleViewer({ onClose, onStarted }: { onClose: () => void; onStarted: () => void }) {
  ...
  return (
    <PageViewer
      ...
      footer={(
        <>
          <span className="field__hint">가게 이름·연락처는 지어낸 예시이고, 사진은 "사진이 들어갈 자리"로 표시했습니다.</span>
          <button className="btn btn--main" onClick={() => void start()} disabled={busy}>
            {busy ? '지금 작업 보관하는 중…' : '이 구성으로 시작하기'}
          </button>
        </>
      )}
    />
